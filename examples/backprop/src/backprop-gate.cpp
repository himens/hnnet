...
int main() {
    ...
    // create net
    Gate gate;
    const auto input_layer  = gate.new_neurons<hNNet::Builtin::BackpropNeuron>(2);
    const auto hidden_layer = gate.new_neurons<hNNet::Builtin::BackpropNeuron>(4);
    const auto output_layer = gate.new_neurons<hNNet::Builtin::BackpropNeuron>(1);
    gate.connect(input_layer, hidden_layer);
    gate.connect(hidden_layer, output_layer);
    const auto hidden_biases = gate.new_neurons<hNNet::Builtin::BackpropNeuron>(4);
    for (const auto &[bias, neuron] : std::views::zip(hidden_biases, hidden_layer)) {
        gate.connect(bias, neuron);
    }
    const auto output_bias = gate.new_neurons<hNNet::Builtin::BackpropNeuron>(1);
